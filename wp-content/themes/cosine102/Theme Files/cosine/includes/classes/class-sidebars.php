 ...
 */
class Cosine_Sidebars extends Cosine_Base
{
	...
	public function enqueue_assets( $hook = null ) {
		...
		wp_localize_script( 'theme-sidebars', '_sidebarSettings', array(
			'button_title'    => esc_html__( 'remove this area', 'cosine' ),
			'confirm_message' => esc_html__( 'Are you sure you want to remove this widget area?', 'cosine' ),
	...
	public function admin_form() {
		...
		$messages = array();
		$messages[1] = esc_html__( 'New custom widget area has been created', 'cosine' );
		$messages[2] = esc_html__( 'Custom widget area has been removed', 'cosine' );

		$errors = array();
		$errors[2] = esc_html__( 'Cannot create custom widget area, please try again', 'cosine' );
		$errors[3] = esc_html__( 'Invalid the ID of widget area', 'cosine' );
		?>
			...
			<div id="sidebars-form" class="widget-liquid-right">
				<form action="<?php echo esc_url( admin_url() ) ?>admin-ajax.php?action=save_custom_sidebar" method="POST" class="widgets-holder-wrap">
					...
					<h3><?php esc_html_e( 'Create Widget Area', 'cosine' ) ?></h3>
					<div class="sidebar-inputs">
						<label for="sidebar-name"><?php esc_html_e( 'Enter name of the widget area:', 'cosine' ) ?></label>
						<input type="text" name="sidebar-name" id="sidebar-name" />
						<input type="submit" class="button button-primary" value="<?php esc_html_e( 'Create', 'cosine' ) ?>" />
					</div>
				</form>
				...
				<?php if ( ! empty( $custom_sidebars ) ): ?>
					<form id="remove-sidebar-form" action="<?php echo esc_url( admin_url() ) ?>admin-ajax.php" method="GET" class="widgets-holder-wrap hide-if-js">
						...
						<h3><?php esc_html_e( 'Remove Custom Widget Area', 'cosine' ) ?></h3>
						<div class="sidebar-inputs">
							<label for="sidebar-id"><?php esc_html_e( 'Select custom widget area to be removed:', 'cosine' ) ?></label>
							...
							<input type="submit" class="button" value="<?php esc_html_e( 'Remove', 'cosine' ) ?>" />
						</div>
					</form>
				<?php endif ?>
			</div>
	...
	public function save_sidebar() {
		if ( isset( $_POST['custom-sidebar'] ) &&
			...
			$custom_sidebars['sidebar-' . uniqid()] = array(
				'name' => $name,
				'description' => sprintf( esc_html__( 'There are widgets for %s', 'cosine' ), $name )
			);
