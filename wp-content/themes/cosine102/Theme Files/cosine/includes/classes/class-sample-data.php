 ...
 */
class Cosine_SampleData extends Cosine_Base
{
	...
	public function admin_menu() {
		add_theme_page( esc_html__( 'Sample Data Installation', 'cosine' ),
						esc_html__( 'Sample Data', 'cosine' ),
	...
	public function admin_page() {
		?>
			...
			<div id="sample-data-installer">
				<h1><?php esc_html_e( 'Sample Data Installation', 'cosine' ) ?></h1>

				<div class="start-screen">
					<p><?php esc_html_e( 'There is following tasks will be run for install sample data:', 'cosine' ) ?></p>
					<ol class="tasks">
						<li><?php esc_html_e( 'Import sample content', 'cosine' ) ?></li>
						<li><?php esc_html_e( 'Download media files', 'cosine' ) ?></li>
					</ol>

					<p>
						<button type="button" id="install-sample-data" class="button-primary"><?php esc_html_e( 'Install Sample Data', 'cosine' ) ?></button>
					</p>
				</div>
				...
				<div class="running-screen">
					<ol class="tasks">
						<li data-task="import-content">
							<span><?php esc_html_e( 'Import sample content', 'cosine' ) ?></span>
							<span class="spinner-text"></span>
						</li>
						<li data-task="download-attachments">
							<span><?php esc_html_e( 'Download media files', 'cosine' ) ?></span>
					...
					<p class="finish-actions">
						<span><?php esc_html_e( 'Congratulation! Sample data has been installed successfully', 'cosine' ) ?></span>
					</p>
				</div>
			</div>
	...
	public function enqueue( $page ) {
		if ( $page == 'appearance_page_sample-data' ) {
			...
			wp_localize_script( 'theme-sample-data', '_sampleDataLocalization', array(
				'confirm_installation' => esc_html__( 'Attention!!! Your existing data will be removed when install sample data. Are you sure you want to install sample data?', 'cosine' )
			) );
	...
	public function import_content( $response, $context ) {
		...
		if ( $context['step'] == 'import-content' && wp_verify_nonce( $context['nonce'], 'sample_data_installation' ) ) {
			...
			if ( ! is_file( $sample_data_path ) )
				throw new Exception( sprintf( esc_html__( 'Sample data file not found: %s', 'cosine' ), $sample_data_path ) );

			if ( ! $this->init_filesystem_api() )
				throw new Exception( esc_html__( 'Cannot request permision for FileSystem API', 'cosine' ) );
